...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.compat.v1.placeholder(tf.float32, 
                                                            shape=(None, self.observation_dim))
    
    if self.discrete:
      self.action_placeholder = tf.compat.v1.placeholder(tf.int32, 
                                                         shape=(None))
    else:
      self.action_placeholder = tf.compat.v1.placeholder(tf.float32, 
                                                         shape=(None, self.action_dim))
    
    self.advantage_placeholder = tf.compat.v1.placeholder(tf.float32, shape=None)
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      mlp_out = build_mlp(self.observation_placeholder, self.action_dim, scope,
                          self.config.n_layers, self.config.layer_size,
                          self.config.activation)

      self.sampled_action = tf.squeeze(tf.multinomial(mlp_out, num_samples=1), 1)

      self.logprob = -tf.nn.sparse_softmax_cross_entropy_with_logits(self.action_placeholder,
                                                                     mlp_out)
    else:
      action_means = build_mlp(self.observation_placeholder, self.action_dim, scope,
                               self.config.n_layers, self.config.layer_size)

      log_std = tf.get_variable("log_std", shape=[1, self.action_dim], trainable=True)

      self.sampled_action = tf.random_normal((1,), 
                                             mean=action_means,
                                             stddev=tf.math.exp(log_std))

      mvn = tf.contrib.distributions.MultivariateNormalDiag(action_means, tf.math.exp(log_std))
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      for i in range(T-1, -1, -1):
        returns[i] = rewards[i] + self.config.gamma * last_Gt
        last_Gt = returns[i]
