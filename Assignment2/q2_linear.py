...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.compat.v1.placeholder(tf.uint8, shape=(None,
                                                           state_shape[0], state_shape[1],
                                                           state_shape[2] * config.state_history))
        self.sp = tf.compat.v1.placeholder(tf.uint8, shape=(None, 
                                                            state_shape[0], state_shape[1],
                                                            state_shape[2] * config.state_history))
        
        self.a = tf.compat.v1.placeholder(tf.int32, shape=(None))
        self.r = tf.compat.v1.placeholder(tf.float32, shape=(None))
        self.done_mask = tf.compat.v1.placeholder(tf.bool, shape=(None))
        self.lr = tf.compat.v1.placeholder(tf.float32, shape=None)
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        state_fl = tf.layers.flatten(state)
        out = tf.contrib.layers.fully_connected(state_fl, num_actions, activation_fn=None,
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        glb = tf.compat.v1.GraphKeys.GLOBAL_VARIABLES
        col_q = tf.compat.v1.get_collection(glb, scope=q_scope)
        col_target = tf.compat.v1.get_collection(glb, scope=target_q_scope)

        ops = [tf.compat.v1.assign(vtar, vq) for vq, vtar in zip(col_q, col_target)]
        ...
        #     v = sess.run(col_q[0])
        #     v1 = sess.run(col_target[0])
        #     print(v, "\n")  # will show you your variable.
        #     print(v1)
    ...
    def add_loss_op(self, q, target_q):
        ...
        max_trgt = tf.math.reduce_max(target_q, axis=1)

        Q_samp = tf.where(self.done_mask, self.r, self.r + self.config.gamma * max_trgt)
        q_red = tf.reduce_sum(tf.one_hot(self.a, num_actions) * q, axis=1)

        self.loss = tf.reduce_mean(tf.squared_difference(Q_samp, q_red))
    ...
    def add_optimizer_op(self, scope):
        ...
        glb = tf.compat.v1.GraphKeys.GLOBAL_VARIABLES
        grad_vars = tf.compat.v1.get_collection(glb, scope=scope)

        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.lr)

        grads_vars = optimizer.compute_gradients(self.loss, var_list=grad_vars) #[(gradients, variables)]

        if self.config.grad_clip:
            grads_vars_clipped = [(tf.clip_by_norm(grad, self.config.clip_val), var) for grad, var in grads_vars]

        self.train_op = optimizer.apply_gradients(grads_vars_clipped)

        grads = [grad for grad, _ in grads_vars_clipped]
